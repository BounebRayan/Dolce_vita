...
const DecorationsSubcategoryPage = () => {
  const params = useParams();
  const { category, subcategory } = params;
  const categoryStr = decodeURIComponent(Array.isArray(category) ? category[0] : category);
  const subcategoryStr = decodeURIComponent(Array.isArray(subcategory) ? subcategory[0] : subcategory);
  ...
  // Get category info
  const categoryInfo = categories.deco.find(cat => cat.type === categoryStr);
  ...
  useEffect(() => {
    ...
    if (categoryStr && subcategoryStr) {
      fetchProducts();
    }
  }, [categoryStr, subcategoryStr, onSale, sortAttribute, sortOrder]);

  return (
    <div className="mx-4 sm:mx-8 lg:mx-12 mt-1 md:mt-2">
      <main className="w-full pl-0 lg:pl-4">
        <h1 className="text-[28px] font-light mb-2">
          {categoryInfo?.text} - {subcategoryStr}
        </h1>
...
export default DecorationsSubcategoryPage;
